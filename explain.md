...
This article is an introduction to and explination of SubsidiaDB. Features and usage will not be covered thoroughly; instead, we will gradually explore the systems and intuitions behind how it works. It's important to know where we are headed, so to reiterate the tagline: SubsidiaDB is a transactional, concurrent, embedded database that utilyzes the filesystem as it's storage engine.
...
I see this database as filling a neglected niche for applications that need persistent storage. As already expounded upon, filesystems by themselves do not offer the ACID guarantees needed for non-trivial resilient applications. Other embedded databases (like SQLite, RocksDB, LMDB, etc.) are more complex, and by using a a single file, prevent truly concurrent multi-process writes. Full DBMSs are heavy on resources and incur higher operational burden; they are also not well suited for applications intending to be distributed and run by others as a single executable. SubsidiaDB is not better than these other solution, but it brings a different set of tradeoffs to the table.
